...
Nine moods, one morphing SVG. A studio preview of the uncoverLAB cat mascot: the illustration was drawn in six states, and this prototype morphs between them and keeps the character alive in each one.
...
3. **Love** — pupils turn into red hearts that pump on a 1.2s lub-dub; body, halo, ears and smile all ride the same beat
...
7. **Worried** — trembles, pupils dart, sparkles in the eyes
8. **Ugh** — leans away, a queasy wave runs along the mouth, a shudder every few seconds
9. **Sleepy** — closed lids and the mouth ride the breath, a lid twitches, zzz drift up

Switch with the chips, click the character, use ← → or 1–9, and space for autoplay. `prefers-reduced-motion` keeps the morph and drops the shaking.

## How the morph works

The six SVGs have different node counts, so paths cannot be interpolated as-is. Every layer (head, eyes, pupils, mouth, jaw line, brow, sparkles) is sampled into a fixed number of points along its outline. Layers that share fixed features across states are sampled between anchors, so the body of the head and the nose inside the mouth get identical points in every state and stay perfectly still while ears and mouth line morph. Fill colours interpolate too, which is how the pupils turn into hearts. Points are interpolated with a spring and drawn as a closed centripetal Catmull-Rom spline, so the outline stays smooth at any zoom.
...
- `components/prototypes/mascotMoodsData.ts` — the nine states' path data, generated from the source SVGs
